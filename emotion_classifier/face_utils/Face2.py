...
class Face:
    ...
    def detect(self, image_list=None):
        ...
        """
        self.__is_image_list_empty(image_list)
    ...
    def cnn_detect(self, image_list=None,
        ...
        检测器返回一个mmod矩形对象，这个对象包含了一个列表的mmod矩形。只需遍历mmod_rectangles对象即可访问这些对象
        ...
        """
        self.__is_image_list_empty(image_list)
        self.__is_path_exists(detector_path)
    ...
    def landmark_detect(self, image_list=None,
        ...
        """
        self.__is_path_exists(shape_predictor_path)
        self.__is_image_list_empty(image_list)
    ...
    def recognition(self, image_list=None, role_image=None,
        ...
        self.__is_image_list_empty(image_list)
        self.__is_path_exists(shape_predictor_path)
        self.__is_path_exists(recognition_model_path)
    ...
    def alignment(self, image_list, shape_predictor_path=configs_shape_predictor_path):
        """图像对齐函数"""
        self.__is_image_list_empty(image_list)
        self.__is_path_exists(shape_predictor_path)
    ...
    def clustering(self, image_list=None, 
        ...
        if not os.path.isdir(output_folder_path):
            os.makedirs(output_folder_path)
        # 保存提取出来的脸部
        print('正在保存最大聚类到脸部文件夹{}'.format(output_folder_path))
    ...
    def save(self, path):
        """ 将图片保存 """
        self.__is_path_exists(path)
        pass
    
    def __is_image_list_empty(self, image_list):
        if image_list is None:
            raise ValueError("image_list为空")

    def __is_path_exists(self, path):
        if os.path.exists(path) is False:
            raise ValueError("路径错误{}".format(path))
